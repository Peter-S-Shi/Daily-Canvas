...
- Milestone 12 verification (synthetic data): TypeScript checking; 107/107 automated tests across 18 suites, including 15-minute-grid and overlap invariants, Avoidance exclusion from Time Blocks, Available Work derivation, Replan `needsReview` flagging without history rewrite, restrained reminder catch-up semantics, and full v1-v8 schema/backup migration and round-trip coverage; production build. `cargo check` and a release Windows/MSVC Tauri build pass locally with the new `tauri-plugin-notification` dependency and its `notification:default` capability. Manually verified in a live browser preview: creating/editing a Time Block, overlap rejection, the Today's Plan summary, Task Detail reminder editing, and the Settings Shortcuts cheat sheet, with zero console errors. GitHub Actions CI on PR #8 (run `35811774014`) confirmed all tiers green: Classify, Core, Desktop Windows/MSVC (66/66 packaged-app smoke checks including the v8 backup restore/export round-trip, the new Plan/Timeline and Settings/Shortcuts screens, shortcut behavior, and the notification boundary; 17/17 installer/upgrade/data-retention smoke checks), and PR Gate.
...
- Branch: `main` has Milestone 11 integrated (`2035711`, PR #6). Milestone 12 is complete on `milestone/12-timeline-desktop-execution`, developed from baseline `2cb9497c720a9c5dd608f46b70e44a2668594ed7`, and open against `main` as **PR #8 (not yet merged)**.
- Current application version: `0.7.0`
- Dexie schema and backup format on Milestone 12: `v8` (still `v7` on `main` until PR #8 merges)
- Current implementation baseline (Milestone 12 branch): Milestone 7.1 + Milestone 8 desktop foundation + Milestone 9 frozen blueprint + Milestone 10 desktop UI migration + Milestone 11 capture/task enrichment + Milestone 12 timeline/desktop execution
...
- v1.0 product scope: approved; Milestones 8–12 complete (of 15) once PR #8 merges; Milestone 13 next, not started
