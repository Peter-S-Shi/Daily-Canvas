...
**v1.0 Desktop Program — Milestone 15 (RC1) In Progress, First RC Batch Landed on an Open PR**
...
**Milestone 15: Daily Canvas v1.0.0 Release Candidate and Delivery — In Progress (RC Batch 1 of N, not merged).**
...
- **`ROADMAP.md` M15 "Planned scope" bullet-by-bullet disposition:**
  ...
  7. *Verify GitHub Release update awareness against release metadata.* — **Open/deferred, by design, pending a human decision.** This repository has zero published GitHub Releases or tags. The four/five-state `updateCheckService`/`desktopAdapter` logic (up-to-date, update-available, no-release/404, network-failure, check-failed) is unit-tested with injected fakes (`updateCheckService.test.ts`) and the packaged-app smoke confirms the real, live (non-mocked) network call settles into one of the defined states without blocking the app. Genuinely exercising the "update-available" state end-to-end against real GitHub Release metadata would require an actual published Release. Per the task brief's guidance, this batch did **not** publish one (a public Release is a visible, hard-to-reverse action). **This is flagged for a human decision**, not decided here: either (a) accept unit/mock coverage as sufficient for RC sign-off and defer live-Release verification to final delivery (when a real v1.0.0 Release is published anyway), or (b) publish a draft/prerelease now specifically to close this gap before RC sign-off.
  8. *Finalize release notes, known limitations, privacy/data-ownership explanation, and end-user documentation.* — **Not done this batch**, out of scope for RC batch 1 as framed (this batch's task list did not include it); flagged as open for a subsequent M15 batch.
  9. *Confirm package version, UI version, docs, schema version, backup version, repository commit, tag, and published artifact all identify the same accepted candidate.* — **Partially covered.** Package version (`package.json`/`tauri.conf.json`/`Cargo.toml`/`Cargo.lock`), UI version (About & Updates reads live), and schema/backup version (v9, documented here as shipping with `1.0.0`) all agree as of commit `ca73026` on this branch. No tag or published artifact/Release exists yet (see item 7) — that identification step is necessarily still open until a Release is actually cut.
- **Release blockers found this batch:** none.
- **Recommendation for the next concrete M15 work batch** (a recommendation, not a decision — final RC sign-off is the user's call): resolve the item-7 open question above (accept unit/mock update-check coverage as sufficient, or deliberately publish a draft/prerelease to close it), then move to item 8 (release notes / known limitations / end-user documentation) as the next batch's primary scope. If item 7 is resolved as "sufficient," nothing else appears to block RC sign-off from an engineering-evidence standpoint based on this audit.
- **Note:** the `/handoff` skill was not used to produce this record — it is reserved for direct user invocation and cannot be triggered programmatically from within this batch. This RC Evidence Record is the durable state record for now; run `/handoff` separately if the formal handoff artifact is also wanted.

## Next Engineering Objective

Continue **Milestone 15: Daily Canvas v1.0.0 Release Candidate and Delivery**, RC batch 2. PR #31 (RC batch 1) is open against `main`, not merged. The immediate open question is the GitHub Release update-awareness verification decision (RC Evidence Record item 7 above); after that, release notes / known limitations / end-user documentation (item 8) is the next substantive scope.
...
- v1.0 product scope: approved; Milestones 8–14 complete (of 15); Milestone 15 in progress (RC batch 1 on open PR #31, not yet merged into `main`) — see the Milestone 15 RC Evidence Record above
