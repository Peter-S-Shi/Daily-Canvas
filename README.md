...
The current codebase is **v0.7.0**, a React/Vite application backed by Dexie/IndexedDB. Milestones 1–12 are complete, including the Tauri 2 Windows desktop foundation, frozen desktop UI blueprint, desktop UI migration, the first v1.0 capture/task-enrichment capabilities, and Day/Week Timeline with Time Blocks and local reminders. The application runs both browser-served and as a packaged Windows desktop app. Milestone 13 is next.
...
- Preserve backup compatibility through format v8, including migration of supported v1-v7 backups.
...
Delivered through Milestone 12: Quick Capture / Inbox, Global Search, Task Notes, one-level Checklist items, richer recurrence rules, Task duration estimates, explicit replanning of unfinished work, optional Day / Week Timeline with Time Blocking, basic local reminders, and the desktop keyboard shortcut set.

Remaining approved capabilities (Milestone 13):
...
- Desktop-native adapters (local files, print, local notifications) are separated from domain services behind `src/desktop/desktopAdapter.ts`; a release-awareness adapter follows in Milestone 13 as that feature is built.
