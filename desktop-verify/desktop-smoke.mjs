...
const tl = Date.now(); let app = await launchApp(exe);
...
check("restore preview parsed the v6 backup", /tasks|Tasks|任务/.test(previewText) || previewText.length > 20, previewText.replace(/\s+/g, " ").slice(0, 160));
...
const files = walk(dataRoot); const total = files.reduce((a, f) => a + f.size, 0);
const idbDir = files.filter((f) => /IndexedDB/i.test(f.p));
console.log(`   ${dataRoot}: ${files.length} files, ${(total / 1e6).toFixed(1)} MB; IndexedDB files: ${idbDir.length}`);
check("user data lives under %LOCALAPPDATA%\\<identifier>\\EBWebView\\...\\IndexedDB", idbDir.length > 0 && info.webviewDataDir.toLowerCase() === join(dataRoot, "EBWebView").toLowerCase(), info.webviewDataDir);
const roaming = existsSync(join(process.env.APPDATA, IDENT)); check("nothing written under %APPDATA% (Roaming)", !roaming);
...
writeFileSync(join(outDir, "results.json"), JSON.stringify({ when: new Date().toISOString(), exe, exeBytes, dataRoot, dataMB: total / 1e6, results }, null, 2));
