...
describe("Milestone 6 lifecycle decisions", () => {
  ...
  it("upgrades a v4 backup with lifecycle defaults while retaining all source records", () => {
    ...
    expect(result.payload.version).toBe(9); expect(result.payload.checkIns).toHaveLength(1); expect(result.payload.taskLifecycles).toEqual([expect.objectContaining({ taskId: task.id, personalBest: 1 })]); expect(result.payload.pausePeriods).toEqual([]); expect(result.payload.meditationEntries).toEqual([]);
  });
});
