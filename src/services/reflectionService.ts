...
export async function saveReflection(input: Omit<DailyReflection, "createdAt" | "updatedAt">): Promise<DailyReflection> {
  ...
  const reflection: DailyReflection = { ...input, emotionIds: [...new Set(input.emotionIds)], templateId: input.templateId ?? existing?.templateId, createdAt: existing?.createdAt ?? now, updatedAt: now };
