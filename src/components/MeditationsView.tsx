...
export function MeditationsView({ selectedId = "" }: { selectedId?: string }) {
  ...
  useEffect(() => { if (selectedId) [...document.querySelectorAll<HTMLElement>("[data-meditation-id]")].find((element) => element.dataset.meditationId === selectedId)?.scrollIntoView({ block: "center" }); }, [selectedId, entries.length]);
  ...
  return (
    <div className="page page-narrow">
      ...
      {selecting && <div className="selection-bar" role="region" aria-label={t("select")}><strong>{t("selectedCount", { count: selected.size })}</strong><button type="button" className="button secondary" disabled={selected.size === entries.length} onClick={() => setSelected(new Set(entries.map((entry) => entry.id)))}>{t("selectAllMeditations")}</button><button type="button" className="button secondary" disabled={selected.size === 0} onClick={() => setSelected(new Set())}>{t("clearAllMeditations")}</button><button type="button" className="button primary" disabled={selected.size === 0} onClick={() => setExportIds([...selected])}>{t("exportSelected")}</button><button type="button" className="button secondary" onClick={closeSelection}>{t("cancelSelection")}</button></div>}
      {entries.length === 0 ? <div className="empty-state"><h3>{t("noMeditations")}</h3><p>{t("noMeditationsHint")}</p><button type="button" className="button secondary" onClick={() => setEditing(null)}>{t("writeFirstMeditation")}</button></div> : <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}><SortableContext items={entries.map((entry) => entry.id)} strategy={verticalListSortingStrategy}><div className="meditation-list">{entries.map((entry) => <SortableMeditationCard key={entry.id} entry={entry} selected={selected.has(entry.id)} highlighted={entry.id === selectedId} selecting={selecting} onSelect={() => toggle(entry.id)} onEdit={() => setEditing(entry)} onDelete={() => remove(entry)} locale={i18n.language}/>)}</div></SortableContext></DndContext>}
