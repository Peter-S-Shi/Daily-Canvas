...
export function TaskEditor({ task, onClose }: TaskEditorProps) {
  ...
  const defaults = useMemo(() => {
    ...
    return {
      title: task?.title ?? "", kind: task?.kind ?? ("habit" as TaskKind), areaId: task?.areaId ?? "", colorOverride: task?.colorOverride ?? "", starred: task?.starred ?? false,
    ...
    <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="task-editor-title">
      ...
      <form className="task-form" onSubmit={save}>
        {step === 1 ? <>
          ...
          <div className="field field-wide"><span>{t("scheduleType")}</span><div className="segmented schedule-segments">{([ ["fixed","fixedSchedule"], ["floating","floatingTask"], ["quota","quotaGoal"] ] as const).map(([value,label]) => <button key={value} type="button" className={form.scheduleMode === value ? "active" : ""} onClick={() => setMode(value)}>{t(label)}</button>)}</div></div>
          {form.scheduleMode !== "floating" && <div className="segmented field-wide" aria-label={t("taskKind")}>{([ ["task","regularTask"], ["habit","goodHabit"], ["avoidance","avoidanceHabit"] ] as const).filter(([value]) => form.scheduleMode === "fixed" || value !== "task").map(([value,label]) => <button key={value} type="button" className={form.kind === value ? "active" : ""} onClick={() => setForm({ ...form, kind: value })}>{t(label)}</button>)}</div>}
          ...
          {form.scheduleMode === "fixed" && form.kind !== "task" && form.recurrenceType === "weekdays" && <div className="field field-wide"><span>{t("weekdays")}</span><div className="weekday-picker">{dayKeys.map((key, day) => <button type="button" key={key} className={form.weekdays.includes(day) ? "selected" : ""} onClick={() => toggleWeekday(day)}>{t(key)}</button>)}</div></div>}
